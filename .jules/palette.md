...
## 2026-06-19 - Settings Group Context Accessibility
**Learning:** In WinUI 3 XAML, setting `AutomationProperties.LabeledBy` on a non-focusable container (like `StackPanel`) is a no-op for assistive technologies because focus never lands on the container. Attempting to force group semantics via `LocalizedControlType="group"` does not change this behavior. Furthermore, explicitly setting `LabeledBy` on individual child controls that already have a specific `AutomationProperties.Name` is an anti-pattern that can cause naming conflicts or regressions in older screen readers (like JAWS or NVDA).
**Action:** Do not attempt to use `AutomationProperties.LabeledBy` on non-focusable layout panels to establish group context, as it has no effect in the UIA reading path. Rely on explicit per-control `AutomationProperties.Name` (e.g., via `x:Uid`) for individual inputs. However, when defining interactive `ListView` items, always set `AutomationProperties.Name="{x:Bind LabelProperty}"` on the root `DataTemplate` element (e.g., `Grid`) to ensure the entire item is properly named for screen readers when focused.
