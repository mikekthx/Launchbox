...
## 2026-05-18 - Tooltip Area Hit-Testing
**Learning:** In WinUI 3 XAML, when expanding the hover target area of a tooltip in a `DataTemplate`, apply `ToolTipService.ToolTip` to the layout container (e.g., `<StackPanel>`) that strictly encapsulates the relevant text. Do NOT apply it to the root container (e.g., `<Grid>`) if it also holds other interactive elements (like buttons), as the tooltip will inappropriately appear when hovering over those unrelated controls. Additionally, do NOT apply `AutomationProperties.Name` to non-interactive structural elements, as this is an accessibility anti-pattern.
**Action:** When adjusting tooltip bounds for UX improvements, evaluate the children of the layout container. If it contains sibling interactive controls, push the tooltip binding only to the sub-container holding the relevant text (e.g. inner `<StackPanel>`) rather than the root element.
