...
## 2026-06-19 - Settings Accessibility and Name Specificity
**Learning:** In WinUI 3 XAML, setting `AutomationProperties.LabeledBy` on individual child controls that already have a specific `AutomationProperties.Name` (e.g., from an `.resw` file via `x:Uid`) can cause naming conflicts or regressions in older screen readers (like JAWS or NVDA). Additionally, attempting to establish group context by setting `LabeledBy` on a non-focusable layout panel (like `StackPanel`) is a no-op because focus never lands on the container.
**Action:** Do not add `AutomationProperties.LabeledBy` to controls that already have a specific `AutomationProperties.Name` set via `x:Uid` - it is redundant and can cause regressions in older AT clients. `LabeledBy` is valid and the established pattern when a focusable control (e.g., `ComboBox`, `TextBox`) does *not* have an `x:Uid`-set Name: use `AutomationProperties.LabeledBy="{Binding ElementName=HeaderName}"` on the control directly. When defining interactive `ListView` items, always set `AutomationProperties.Name="{x:Bind LabelProperty}"` on the root `DataTemplate` element (e.g., `Grid`) to ensure screen readers name the item correctly.
