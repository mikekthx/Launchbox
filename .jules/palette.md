...
**Learning:** In WinUI 3 XAML, setting `AutomationProperties.LabeledBy` on a non-focusable container (like `StackPanel`) is a no-op for assistive technologies, and setting it on individual controls may conflict with their explicitly defined `AutomationProperties.Name`.
**Action:** To provide group context for a section of controls (e.g., a group of hotkey inputs), apply `AutomationProperties.AutomationControlType="Group"` and `AutomationProperties.LabeledBy="{Binding ElementName=HeaderName}"` to the parent container that wraps both the header and the controls. Additionally, when defining interactive `ListView` items, set `AutomationProperties.Name="{x:Bind LabelProperty}"` on the root `DataTemplate` element (e.g., `Grid`) to ensure the item is properly named for screen readers.
